...
```
Open CSV files as tables. Create a new CSV from a folder, view with a filter, pin the last row or first column, edit cells with Tab or Enter, copy or paste, and export filtered rows.
```
...
Edit mode updates cells, rows, and columns, then saves. Tab and Enter move between cells without breaking IME composition.
