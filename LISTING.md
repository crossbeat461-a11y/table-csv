...
```
Open CSV files as tables. Create a new CSV from a folder, view with a filter, pin the last row or first column, resize columns, undo edits, edit cells with Tab or Enter, copy or paste, and export filtered rows.
```
...
Edit mode updates cells, rows, and columns, then saves. Tab and Enter move between cells without breaking IME composition. Undo reverses the last edit. Drag a column header edge to set the width.
